"""
Plot posterior residual summary for many injections.

Usage
-----
python ligo_summary_residuals_plot.py \
    --posterior-dir   /path/to/results/ \
    --injections-file /path/to/injections.txt \
    --outfig          /path/to/residuals_summary.png

Example:
python ligo_summary_residuals_plot.py --posterior-dir /home/xavier.garcia-sabat/Lensing/pesummaries_merge_all/ --injections-file /home/xavier.garcia-sabat/Lensing/ler_data/n_lensed_params_bbh_filtered.txt --outfig /home/xavier.garcia-sabat/Lensing/pesummaries_merge_all/residuals_summary.png
"""
...
PARAMETER_CONFIG = {
    "chirp_mass": {
        ...
        "title":  r"$\mathcal{M}$",
        "xlabel": r"Injected $\mathcal{M}\,[M_\odot]$",
        "relative": False,
    },
    "mass_ratio": {
    ...
    "chi_eff": {
        ...
        "title":  r"$\chi_{\mathrm{eff}}$",
        "xlabel": r"Injected $\chi_{\mathrm{eff}}$",
        "relative": False,
    },
    "chi_p": {
        "posterior": ["chi_p", "xp"],
        ...
        "title":  r"$\chi_{\mathrm{p}}$",
        "xlabel": r"Injected $\chi_{\mathrm{p}}$",
        "relative": False,
    },
    "a_1": {
        "posterior": ["a_1", "a1"],
    ...
    "luminosity_distance_1": {
        ...
        "title":  r"$D_{L,1}$",
        "xlabel": r"Injected $D_{L,1}\,[\mathrm{Mpc}]$",
        "relative": True,
        "image_index": 1,
    },
    "luminosity_distance_2": {
        ...
        "title":  r"$D_{L,2}$",
        "xlabel": r"Injected $D_{L,2}\,[\mathrm{Mpc}]$",
        "relative": True,
        "image_index": 2,
    },
}
...
def load_snr_table(path: Path) -> dict[int, dict[str, float]]:
    ...
    for row in rows:
        inj = int(row["inj"])
        table[inj] = {
            "optimal_img1": float(row["optimal_img1"]),
            "optimal_img2": float(row["optimal_img2"]),
            "optimal_joint": float(row["optimal_joint"]),
            "matched_img1": float(row["matched_img1"]),
            "matched_img2": float(row["matched_img2"]),
            "matched_joint": float(row["matched_joint"]),
            ...
            "matched_joint_p5": float(row["matched_joint_p5"]),
            "matched_joint_p50": float(row["matched_joint_p50"]),
            "matched_joint_p95": float(row["matched_joint_p95"]),
        }
    return table
...
def summarize_event(posterior, injections, snr_table, parameter, event_name):
    ...
    if cfg.get("external_snr", False):
        ...
        p5 = row["optimal_joint_p5"]
        p50 = row["optimal_joint_p50"]
        p95 = row["optimal_joint_p95"]
        return {
            "inj": inj_value,
            "y5": (p5 - inj_value) / scale,
            "y50": (p50 - inj_value) / scale,
            "y95": (p95 - inj_value) / scale,
        }

    post_values = get_posterior_field(posterior, cfg["posterior"])
    p5, p50, p95 = np.percentile(post_values, [5.0, 50.0, 95.0])
...
def collect_results(events, injections, snr_table, parameters):
    ...
    for parameter, rows in collected.items():
        if not rows:
            continue
        rows = sorted(rows, key=lambda r: r["inj"])
...
def plot_residuals(results, parameters, outfig):
    ncols = 2
    ...
    for ax, parameter in zip(axes, parameters):
        ...
        x = data["inj"].astype(float)
        y = data["y50"].astype(float)
        yerr = np.vstack([
            y - data["y5"].astype(float),
            data["y95"].astype(float) - y,
        ])
        ...
        ax.set_title(PARAMETER_CONFIG[parameter]["title"])
        ax.set_xlabel(PARAMETER_CONFIG[parameter]["xlabel"])
...
def parse_args():
    parser = argparse.ArgumentParser(description="Plot posterior residual summary.")
    parser.add_argument(
    ...
    parser.add_argument(
        "--outfig", default="ligo_summary_residuals_plot.png",
...
def main():
    ...
    plot_residuals(results, args.parameters, Path(args.outfig))
